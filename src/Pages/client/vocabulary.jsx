...
import { createTopicAPI, updateTopicAPI, deleteTopicAPI } from '../../services/apiVocab';
...
const Vocabulary = () => {
  ...
  const handleDeleteTopic = async (topicId) => {
    if (!window.confirm("Bạn có chắc muốn xóa chủ đề này? Tất cả từ vựng trong chủ đề cũng sẽ bị xóa.")) {
      return;
    }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {topics.length === 0 && (
              <p className="text-slate-500 text-sm">Chưa có chủ đề nào. Hãy thêm chủ đề mới!</p>
            )}
          </div>
        </div>
        ...
        {selectedTopic && (
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold text-slate-700">Từ vựng: {selectedTopic.nameTopic}</h2>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200">
                  <thead className="bg-slate-100">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider">Từ vựng</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider">Loại từ</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider">Phonetic</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider">Nghĩa</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider">Ví dụ</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider">Thao tác</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-slate-200">
                    {filteredVocabularies.map(vocab => (
                      <tr key={vocab.id}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">{vocab.word}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">{vocab.type}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">{vocab.phonetic}</td>
                        <td className="px-6 py-4 text-sm text-slate-700">{vocab.meaning}</td>
                        <td className="px-6 py-4 text-sm text-slate-700">{vocab.example}</td>
        ...
        {!selectedTopic && topics.length > 0 && (
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            <p className="text-slate-600">Vui lòng chọn một chủ đề để xem từ vựng.</p>
          </div>
        )}
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewVocabulary({...newVocabulary, type: e.target.value})}
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Phonetic</label>
                  <input
                    type="text"
                    value={newVocabulary.phonetic}
                    onChange={(e) => setNewVocabulary({...newVocabulary, phonetic: e.target.value})}
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Nghĩa *</label>
                  <input
                    type="text"
              ...
              <div className="flex justify-end space-x-2 mt-6">
                ...
                <button
                  ...
                  disabled={!newVocabulary.word.trim() || !newVocabulary.meaning.trim()}
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setVocabToEdit({...vocabToEdit, word: e.target.value})}
                    className="w-full p-2 border border-slate-300 rounded-md"
                    autoFocus
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={vocabToEdit.type}
                    onChange={(e) => setVocabToEdit({...vocabToEdit, type: e.target.value})}
                    className="w-full p-2 border border-slate-300 rounded-md"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setVocabToEdit({...vocabToEdit, phonetic: e.target.value})}
                    className="w-full p-2 border border-slate-300 rounded-md"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setVocabToEdit({...vocabToEdit, meaning: e.target.value})}
                    className="w-full p-2 border border-slate-300 rounded-md"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setVocabToEdit({...vocabToEdit, example: e.target.value})}
                    className="w-full p-2 border border-slate-300 rounded-md"
                    rows="2"
                  />
                </div>
              ...
              <div className="flex justify-end space-x-2 mt-6">
                ...
                <button
                  ...
                  disabled={!vocabToEdit.word.trim() || !vocabToEdit.meaning.trim()}
                  className={`px-4 py-2 rounded-md ${
