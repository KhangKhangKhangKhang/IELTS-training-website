...
// YES_NO_NOT_GIVEN
// BE metadata: { statement: string, correctAnswer: 'YES' | 'NO' | 'NOT_GIVEN' }
const OPTIONS = ["YES", "NO", "NOT_GIVEN"];

const defaultValue = () => ({
  statement: "",
  correctAnswer: "YES",    type: "YES_NO_NOT_GIVEN",
  });

const YesNoNotGivenForm = ({ value = defaultValue(), onChange }) => {
    ...
    <div className="space-y-2">
      <label className="block">
        <span className="text-[11px] font-extrabold uppercase tracking-wide text-[#64748b] block mb-1.5">
          Statement (the opinion/claim to evaluate)
        </span>
        <Input.TextArea
          value={value.statement}
          onChange={(e) => update({ statement: e.target.value })}
          rows={2}
          placeholder="The statement the student must judge as YES / NO / NOT GIVEN"
        />
      </label>
      <div>
        <span className="text-[11px] font-extrabold uppercase tracking-wide text-[#64748b] block mb-1.5">
...
export default YesNoNotGivenForm;
