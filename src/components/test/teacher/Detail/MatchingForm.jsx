...
const MatchingForm = ({ idGroup, groupData }) => {
  ...
  useEffect(() => {
    // Assuming options are passed in groupData.properties for matching questions
    if (groupData?.properties?.options) {
      setMatchingOptions(groupData.properties.options);
        ...
        setMatchingOptions(['Option A', 'Option B', 'Option C', 'Option D']);
    }
  ...
  return (
    <div className="space-y-4">
        <div className="p-4 bg-blue-50 border border-blue-200 rounded">
            ...
            <p className="text-sm text-gray-700">Select one of the following options for each question below.</p>
            <div className="flex flex-wrap gap-2 mt-2">
                {matchingOptions.map(opt => <span key={opt} className="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded">{opt}</span>)}
            </div>
        </div>
      ...
      {questions.length === 0 ? (
        ...
        questions.map((q, index) => (
          <div key={q.idQuestion} className="border p-4 rounded bg-white">
            <div className="font-semibold mb-2">Câu {q.numberQuestion}: {q.content}</div>
            <div className="mt-3 flex items-center gap-3">
              ...
              >
                {matchingOptions.map(opt => <Option key={opt} value={opt}>{opt}</Option>)}
              </Select>
            </div>
          </div>
        ))
