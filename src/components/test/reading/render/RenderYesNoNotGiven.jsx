    ...
    <div className={containerClass}>
          ...
          <SelectContent className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 shadow-xl z-50">
            {options.map((opt) => (
              <SelectItem
                key={opt}
                value={opt}
                className="focus:bg-blue-50 dark:focus:bg-blue-900/30 focus:text-blue-700 dark:focus:text-blue-400 cursor-pointer py-2.5 text-slate-800 dark:text-slate-200"
              >
                {opt}
              </SelectItem>
            ))}
      ...
      {isReviewMode && !isCorrect && (
        <div className="mt-4 ml-10 text-sm flex items-center gap-2">
          ...
          <span className="px-3 py-1 bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-400 font-bold rounded-lg border border-green-200 dark:border-green-700 shadow-sm">
            {correctAnswerText}
          </span>
        </div>
      )}
