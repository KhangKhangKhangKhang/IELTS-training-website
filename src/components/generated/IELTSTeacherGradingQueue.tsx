...
const TABS: { key: TabKey; label: string; icon: string }[] = [
  { key: "PENDING", label: "Tất cả", icon: "📥" },
  { key: "CLAIMED", label: "Đã nhận", icon: "📌" },
  { key: "IN_PROGRESS", label: "Đang chấm", icon: "✍️" },
  { key: "COMPLETED", label: "Lịch sử", icon: "✅" },
];
...
export const IELTSTeacherGradingQueue = () => {
  ...
  const handleUnclaim = (t: Ticket) =>
    showToast(`Đã trả lại bài của ${t.student}`);
            ...
            <table className="w-full text-left min-w-[640px]">
              <thead>
                <tr className="text-[10px] font-bold uppercase tracking-wide text-[#64748b] border-b-2 border-[#e6e6ed]">
                  ...
                  {tab === "COMPLETED" && (
                    <th className="py-2.5 pr-3">Band GV</th>
                  )}
              ...
              <tbody>
                    ...
                    <td className="py-3 text-right whitespace-nowrap">
                      ...
                      {t.status === "CLAIMED" && (
                        <div className="inline-flex gap-1.5">
                          <PillButton
                            variant="ghost"
                            onClick={() => handleUnclaim(t)}
                          >
                            Trả lại
                          </PillButton>
                          <PillButton
                            variant="cyan"
                            onClick={() => setScoring(t)}
                          >
                            Bắt đầu chấm
                          </PillButton>
                        </div>
                      ...
                      {t.status === "COMPLETED" && (
                        <PillButton variant="ghost">Xem lại</PillButton>
                      )}
                    </td>
                  </tr>
                ))}
                {rows.length === 0 && (
                  <tr>
                    <td
                      colSpan={7}
                      className="py-12 text-center text-[#94a3b8] font-bold"
                    >
                      Không có ticket nào.
                    </td>
                  </tr>
