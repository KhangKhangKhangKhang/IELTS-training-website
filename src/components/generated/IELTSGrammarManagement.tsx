...
const SEED_CATEGORIES: Category[] = [
  {
    id: "cat-1",
    name: "Tenses",
    description: "12 thì trong tiếng Anh — cách dùng, dấu hiệu nhận biết.",
    count: 12,
    isSystem: true,
  },
  {
    id: "cat-2",
    name: "Conditionals",
    description: "Câu điều kiện — 4 loại và mixed conditionals.",
    count: 6,
    isSystem: true,
  },
  {
  ...
  },
  {
    id: "cat-4",
    name: "Passive voice",
    description: "Cấu trúc bị động trong học thuật và giao tiếp.",
    count: 5,
    isSystem: true,
  },
  {
    ...
    description: "Tường thuật lời nói — backshift, đại từ, thời gian.",
    count: 7,
    isSystem: false,
    ownerName: "Cô Mai",
  },
  {
    ...
    description: "Mạo từ a/an/the và các từ hạn định.",
    count: 9,
    isSystem: true,
  },
];

const SEED_GRAMMARS: Grammar[] = [
  {
    id: "g-1",
    ...
    exerciseCount: 18,
    description: "Phân biệt 2 thì khi nói về hành động trong quá khứ và liên hệ với hiện tại.",
  },
  {
    id: "g-2",
    ...
    categories: ["cat-2"],
    exerciseCount: 15,
    description: "Câu điều kiện loại 1 và 2 — when to use which, common mistakes.",
  },
  {
    id: "g-3",
    ...
    categories: ["cat-2"],
    exerciseCount: 20,
    description: "Loại 3 và hỗn hợp — diễn tả giả định trái sự thật trong quá khứ và hiện tại.",
  },
  {
    id: "g-4",
    ...
    categories: ["cat-3"],
    exerciseCount: 22,
    description: "must / might / could / can't khi suy đoán về quá khứ và hiện tại.",
  },
  {
    id: "g-5",
    ...
    categories: ["cat-4"],
    exerciseCount: 24,
    description: "Khi nào nên dùng bị động, các cấu trúc nâng cao cho Writing Task 1.",
  },
  {
    id: "g-6",
    ...
    categories: ["cat-5"],
    exerciseCount: 20,
    description: "Tường thuật lại lời nói — chuyển đổi thì, đại từ, thời gian.",
  },
  {
    id: "g-7",
    ...
    categories: ["cat-6"],
    exerciseCount: 14,
    description: "Quy tắc dùng mạo từ a, an, the trong câu đơn giản.",
  },
];

const LEVEL_META: Record<
  Grammar["level"],
  { label: string; tone: string; cefr: string }
> = {
  Low: { label: "Cơ bản", tone: "bg-[#d1fae5] text-[#047857]", cefr: "A2-B1" },
  Mid: { label: "Trung bình", tone: "bg-[#cffafe] text-[#0e7490]", cefr: "B1-B2" },
  High: { label: "Cao", tone: "bg-[#fef3c7] text-[#b45309]", cefr: "B2-C1" },
  Great: { label: "Nâng cao", tone: "bg-[#fff1f2] text-[#e11d48]", cefr: "C1-C2" },
};
...
function StackedButton({
  ...
  children: React.ReactNode;
  tone?: "indigo" | "coral" | "cyan" | "amber" | "emerald" | "rose" | "ghost";
  size?: "sm" | "md" | "lg";
  className?: string;
  onClick?: () => void;
}) {
...
function StatCard({
  ...
  label: string;
  value: number;
  tone?: "indigo" | "emerald" | "amber" | "coral";
}) {
...
export const IELTSGrammarManagement = () => {
  const [tab, setTab] = useState<"categories" | "grammars">("grammars");
  const [categories, setCategories] = useState<Category[]>(SEED_CATEGORIES);
  const [grammars, setGrammars] = useState<Grammar[]>(SEED_GRAMMARS);
  const [search, setSearch] = useState("");
  const [levelFilter, setLevelFilter] = useState<"all" | Grammar["level"]>(
    "all"
  );
  const [editCategory, setEditCategory] = useState<Category | null>(null);
  const [editGrammar, setEditGrammar] = useState<Grammar | null>(null);
  const [assignGrammar, setAssignGrammar] = useState<Grammar | null>(null);
  ...
  const stats = {
    ...
    high: grammars.filter((g) => g.level === "High" || g.level === "Great")
      .length,
      ...
      <div className="mx-auto w-full max-w-6xl space-y-5">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-[#64748b] font-medium">
                Tạo và quản lý chủ đề + điểm ngữ pháp IELTS cho hệ thống
              </p>
            </div>
            <StackedButton tone="indigo" size="lg">
              + Tạo mới
            </StackedButton>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard
            ...
            tone="indigo"
          />
          <StatCard icon="🌱" label="Cơ bản" value={stats.low} tone="emerald" />
          <StatCard
            icon="🌿"
            label="Trung bình"
            value={stats.mid}
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3 mb-4">
              ...
              <input
                ...
                placeholder={
                  tab === "grammars"
                    ...
                    : "Tìm theo tên hoặc mô tả category..."
                }
            ...
            {tab === "grammars" && (
              <div className="flex gap-1.5 flex-wrap">
                {(["all", "Low", "Mid", "High", "Great"] as const).map((lv) => (
                  <button
                    key={lv}
                  ...
                  >
                    {lv === "all" ? "Tất cả" : LEVEL_META[lv].label}
                  </button>
                ))}
              </div>
            )}
              ...
              filteredGrammars.length ? (
                filteredGrammars.map((g) => (
                  <GrammarRow
                    ...
                    onDelete={() => {
                      if (confirm(`Xóa "${g.title}"?`))
                        setGrammars((p) => p.filter((x) => x.id !== g.id));
                    }}
              ...
              filteredCategories.map((c) => (
                <CategoryRow
                  ...
                  onDelete={() => {
                    if (
                      confirm(
                        `Xóa category "${c.name}"? Tất cả grammar bên trong sẽ bị ảnh hưởng.`
                      )
                    )
        ...
        {editGrammar && (
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="flex gap-1.5 flex-wrap">
                {(["Low", "Mid", "High", "Great"] as const).map((lv) => (
                  <button
                    key={lv}
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <StackedButton tone="indigo" onClick={() => setEditGrammar(null)}>
                Lưu thay đổi
              </StackedButton>
            </div>
          </div>
        ...
        {editCategory && (
          <div className="space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <StackedButton tone="indigo" onClick={() => setEditCategory(null)}>
                Lưu thay đổi
              </StackedButton>
            </div>
          </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-[#64748b]">
                        {c.isSystem ? "Hệ thống" : c.ownerName || "Của tôi"} ·{" "}
                        {c.count} điểm
                      </div>
                    </div>
                  </label>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <StackedButton
                ...
                onClick={() => {
                  setGrammars((p) =>
                    p.map((g) =>
                      g.id === assignGrammar.id ? assignGrammar : g
                    )
                  );
